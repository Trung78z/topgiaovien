...
export default function LoTrinhCourse() {
  const [active, setActive] = useState(1);
  const [dataActive, setDataActive] = useState(steps[0]);
  const handleChangeStep = (data) => {
    setActive(data);
    const update = steps.find((item) => item.id === data);
  ...
  return (
    <div className="relative bg-[#EAEBF1] py-10">
      <div className="container space-y-6 rounded-lg bg-background p-2 px-2 sm:p-10">
        ...
        <div>
          <div className="flex flex-col items-center md:p-4">
            <div className="w-full max-w-4xl">
              <div className="relative hidden w-full grid-cols-2 sm:grid md:grid-cols-5">
                ...
                {steps.map((item) => (
                  ...
                  >
                    ...
                    >
                      <div className="mt-2 text-[16px]">{item.title}</div>
                      <div
                        className={cn(
        ...
        <div className="grid grid-cols-1 gap-4 transition-all delay-500 duration-700 ease-in-out sm:grid-cols-5 sm:gap-10">
          <SubjectOfStudy dataActive={dataActive} />
          <GoalLearn dataActive={dataActive} />
        </div>
      </div>
    </div>
  );
}
const SubjectOfStudy = ({ dataActive }) => {
  ...
  return (
    <>
      ...
      >
        ...
        {dataActive.doiTuongHoc.map((item, index) => (
          <li className="text-neutral-555F6D" key={`${item}+${index}`}>
            {item}
          </li>
        ))}

        <li className="text-neutral-555F6D">{dataActive.thoiGianHoc}</li>
      </ul>
    </>
  );
};

const GoalLearn = ({ dataActive }) => {
  const [animationClass, setAnimationClass] = useState("");
  ...
  return (
    <>
      ...
      >
        ...
        {dataActive.mucTieuKhoaHoc.map((item, index) => (
          <li className="text-neutral-555F6D" key={`${item}+${index}`}>
            {item}
          </li>
        ))}
