...
export default function CareTeacher() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        const update = res.msg.filter((item) => item.teacherId !== null);
            ...
            <TableBody className="max-h-[65vh]">
              {data.map((item, key) => (
                <TableRow key={item?.id}>
                  ...
                  <TableCell className="font-medium">
                    {item?.teacher.courseSubCategory.content}
                  </TableCell>
                  <TableCell className="font-medium">
                  ...
                  <TableCell className="relative font-medium">
                    <div className="flex justify-center text-center">
                      <Link
                        to={`${import.meta.env.VITE_API_URL_WEB}/giao-vien/giao-vien-${item.teacher.id}+${item.teacher.slug}`}
                        target="_blank"
