...
import { editReviewVideo, getReviewVideoById } from "@/services/review-video";
...
const schema = z.object({
  ...
  review: z
    .instanceof(File, { message: "Vui lòng thêm hình ảnh review!" })
    .optional(),
});

export default function EditReviewVideo() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await getReviewVideoById(slug);
        setData(res.msg);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await editReviewVideo(slug, formData);
      Swal.fire({
        icon: "success",
        ...
        <div className="container grid grid-cols-1 items-center gap-4 px-2 sm:max-w-screen-lg sm:grid-cols-2 sm:px-10">
          ...
          <div className="col-span-1 space-y-6">
            <div className="relative flex flex-col gap-2 pl-2 pt-4">
              ...
              {watch("review") === undefined && data?.image && (
                <Link
                  to={`${API_URL}/api/file/review/${data?.image}`}
                  target="_blank"
                >
                  {data?.image}
                </Link>
