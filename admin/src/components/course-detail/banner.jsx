...
export default function BannerELearning({
  props,
  image,
  handleChange,
  handleImageChange,
  ...
  return (
    <div className="relative bg-[#EAEBF1]">
      <div className="container flex flex-wrap items-center justify-evenly gap-2 px-2 pt-10 md:flex-nowrap">
        <div className="relative flex w-full flex-col items-center space-y-6 sm:items-start md:w-[750px]">
          <h1 className="flex flex-col text-[40px] font-semibold text-primary-500">
            Khóa học
            <input
              type="text"
              value={props?.title}
              ...
              className="rounded-xl border-2 px-2 text-2xl sm:text-[52px]"
            />
          </h1>

          <ul className="mt-4 list-inside list-disc text-primary-500 sm:pl-10">
            {props?.content
              ?.trim()
              .split("\n")
              .map((line, index) => (
            ...
            <textarea
              ...
              id="content"
              value={props?.content}
              className="min-w-[300px] max-w-screen-sm rounded-md border p-1 sm:min-w-96"
              onChange={handleChange}
            ></textarea>
            ...
            <li className="text-primary-500">
                Chương trình học tinh gọn, đảm bảo khả năng tư duy & sử dụng ngôn
                ngữ linh hoạt
            </li> */}
          </ul>
        ...
        <div className="relative">
          ...
          >
            <Button
              ...
              variant="outline"
              disabled={true}
              className="absolute rounded-full border-none bg-background text-red-500 hover:text-red-600"
            >
              <MdEdit className="h-8 w-8" />
            </Button>
            <img
              src={image || `${API_URL}/api/file/course/${props?.image}`}
              width={442}
              height={519}
              alt="error"
            ...
            <h3 className="flex-shrink-0 text-[20px] font-semibold text-primary-500">
              <div>
                <input
                  ...
                  value={props?.review}
                  onChange={handleChange}
                  className="editText w-max max-w-20 text-center"
                />
                <span> review</span>
              </div>
            </h3>
