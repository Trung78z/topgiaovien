...
const dataSearch = [
  { id: 1, name: "Giáo viên tiếng anh", value: "English", active: true },
  { id: 2, name: "Giáo viên tiếng Trung", value: "China", active: false },
  { id: 3, name: "Đội ngũ nước ngoài", value: "Khác", active: false },
];

export default function GiaoVienTinhHoa({ props }) {
  const [dataCheck, setDataCheck] = useState(dataSearch);
  ...
  const handleChangeActive = (id, value) => {
    ...
    const filter = dataOld.filter(
      (item) => item.teacher.toLowerCase() === value.toLowerCase(),
    );
    setData(filter);
    ...
    <>
      {data && (
        <div className="container mx-auto px-0">
          ...
          <div className="mx-auto grid grid-cols-1 gap-2 pt-10 sm:max-w-screen-sm sm:grid-cols-3 sm:gap-4">
            {dataCheck.map((row) => {
              return (
                <Button
                  className={cn(
                    "mx-auto w-max rounded-lg border-2 p-2 px-4 font-medium",
                    row.active
                  ...
                  type="button"
                  key={`${row.id}`}
                  onClick={() => handleChangeActive(row?.id, row?.value)}
                >
                  {row.name}
                </Button>
              );
          ...
          {dataActive ? (
            <div
              className="flex flex-wrap gap-x-10 rounded-lg bg-white px-2 md:px-0 lg:flex-nowrap"
              ref={targetRef}
            >
              <div className="flex w-full flex-col pt-4 lg:w-1/3">
                <div className="flex w-full items-center justify-center py-4">
                  <img
                    ...
                    width={500}
                    height={500}
                    className="h-80 w-full items-center rounded-3xl object-cover"
                  />
                </div>
              ...
              <div className="mt-8 flex w-full flex-col justify-between sm:min-h-[64vh] lg:min-h-[98vh] lg:w-2/3">
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4">
                  {data.slice(startIndex, startIndex + itemsToShow).map((_) => (
                    <Button
                      onClick={() => handleSelection(_.id)}
                      key={_.id}
                      className={cn(
                        `flex flex-col items-center rounded-md border-2 px-4 py-10`,
                        _.id === dataActive?.id
